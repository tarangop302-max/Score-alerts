...
const JSR_ROLE_ID = process.env.JSR_ROLE_ID || ""; // your JSR role ID
...
client.once("ready", async () => {
  ...
  setInterval(() => channel.send("🟢 BOT STILL ONLINE 🚀").catch(() => {}), 30*60*1000);

  // Main leaderboard check
  setInterval(async () => {
    try {
      ...
      for (const p of players) {
        ...
        if (isJSR(p.name)) {
          if (p.score >= 20000 && !jsr20.has(p.name)) {
            await channel.send({ content: `<@&${JSR_ROLE_ID}> HELP NOW!`, embeds: [helpEmbed(p)] });
            jsr20.add(p.name);
          }
          if (p.score >= 50000 && !jsr50.has(p.name)) {
            await channel.send({ content: `<@&${JSR_ROLE_ID}> 🚨 URGENT HELP REQUIRED 🚨`, embeds: [helpEmbed(p)] });
            jsr50.add(p.name);
          }
        }
